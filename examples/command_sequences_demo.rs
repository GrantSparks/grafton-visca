...
use grafton_visca::{
    camera::{Camera, profiles::PTZOpticsG2, CommandBuilderExt, units::Degrees},
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let init_results = camera.commands()
        .power_on()
        .pan_tilt_home()
        .zoom_stop()
        .focus_auto()
    ...
    println!("  ✓ Initialization complete ({} commands executed)\n", init_results.len());
    ...
    let movement_results = camera.commands()
        ...
        .pan_tilt_to_degrees(180.0, 15.0,
    ...
    println!("  ✓ Image adjustments complete ({} commands)\n", image_results.len());
    ...
    let query_results = camera.commands()
    ...
    println!("  ✓ Dynamic sequence complete ({} commands)\n", dynamic_results.len());
    ...
    let smooth_results = camera.commands()
        .zoom_out(ZoomSpeed::new(1).unwrap())  // Start slow zoom out
        .pan_tilt_relative_degrees(30.0, 0.0,   // While panning right
    ...
    println!("  ✓ Smooth transition complete ({} commands)", smooth_results.len());
