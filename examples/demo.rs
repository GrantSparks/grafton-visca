...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    send_command_and_wait(&mut transport, &ExposureCommand { mode: ExposureMode::Manual })?;
    ...
    send_command_and_wait(&mut transport, &WhiteBalanceCommand { mode: WhiteBalanceMode::Auto })?;
    ...
    send_command_and_wait(&mut transport, &ImageFlipCombinedCommand { mode: ImageFlipMode::Horizontal })?;
    thread::sleep(Duration::from_secs(1));
    
    println!("Resetting image flip...");
    send_command_and_wait(&mut transport, &ImageFlipCombinedCommand { mode: ImageFlipMode::Off })?;
