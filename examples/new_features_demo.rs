...
use grafton_visca::{
    camera::{
        ...
        Camera, CommandBuilderExt, CameraExtension, DiagnosticsExt,
    },
    camera_pool::{CameraInfo, CameraPool, PoolConfig},
    transport::{resilient::{ResilientTransport, ResilienceConfig, ResilienceEvent}, UdpTransport},
    Error,
};
...
async fn camera_pool_demo() -> Result<(), Error> {
    ...
    let result = pool.with_camera_async("cam1", |camera| async move {
        camera.power_state_async().await
    }).await;
    
    match result {
        Ok(power_on) => println!("  Camera 1 power state: {}", if power_on { "ON" } else { "OFF" }),
    ...
    let results = pool.with_all_cameras_async(|camera| async move {
        camera.home_async().await
    }).await;
    ...
    for stats in pool.get_all_stats() {
        println!("  {} - Success: {}, Failed: {}, Healthy: {}", 
            stats.info.id, 
    ...
    println!("  Removed {} unhealthy, {} stale cameras", 
...
async fn command_builder_demo() -> Result<(), Error> {
    ...
    let movement_results = camera.commands()
        .pan_tilt_to_degrees(45.0, 0.0, 
    ...
    println!("  ✓ Movement sequence completed ({} commands)", movement_results.len());
    
    // Demonstrate concurrent execution
    println!("\nExecuting concurrent commands:");
    let concurrent_results = camera.commands()
        .custom(
    ...
    println!("  ✓ {} of {} concurrent queries succeeded", successful, concurrent_results.len());
...
async fn extension_traits_demo() -> Result<(), Error> {
    ...
    println!("  Pan/Tilt: {}", if test_result.pan_tilt_ok { "✓" } else { "✗" });
    println!("  Zoom: {}", if test_result.zoom_ok { "✓" } else { "✗" });
    println!("  Focus: {}", if test_result.focus_ok { "✓" } else { "✗" });
    println!("  Exposure: {}", if test_result.exposure_ok { "✓" } else { "✗" });
    
    // Use ScriptingExt trait
    use grafton_visca::camera::extensions::{ScriptingExt, MovementStep, MovementAction};
    
    println!("\nExecuting movement script:");
    let script = vec![
        MovementStep {
            action: MovementAction::PanTilt { pan: 0.0, tilt: 0.0 },
            label: Some("Home".to_string()),
        },
        MovementStep {
            action: MovementAction::Wait { duration: Duration::from_secs(1) },
            label: Some("Pause".to_string()),
        },
        MovementStep {
            action: MovementAction::PanTilt { pan: 90.0, tilt: 15.0 },
