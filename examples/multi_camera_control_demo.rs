...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (id, is_healthy) in health_results {
        println!("  {}: {}", id, if is_healthy { "✓ Healthy" } else { "✗ Unhealthy" });
    }
    
    // Example 1: Initialize all cameras to home position
    println!("\nExample 1: Moving all cameras to home position...");
    let home_results = pool.with_all_cameras_async(|camera| async move {
    ...
    for (camera_id, (pan, tilt)) in positions {
        let result = pool.with_camera_async(camera_id, |camera| async move {
    ...
    let preset_results = pool.with_all_cameras_async(|camera| async move {
        camera.preset_set_async(PresetNumber::new(1).unwrap()).await
    }).await;
    
    let successful = preset_results.values().filter(|r| r.is_ok()).count();
    println!("  ✓ Saved preset 1 on {}/{} cameras", successful, preset_results.len());
    ...
    let _ = pool.with_all_cameras_async(|camera| async move {
    ...
    for (id, (pan, tilt)) in fan_positions {
        let _ = pool.with_camera_async(id, |camera| async move {
            camera.pan_tilt_to_async(Degrees(pan), Degrees(tilt)).await
        }).await;
    }
    tokio::time::sleep(Duration::from_secs(2)).await;
    ...
    let _ = pool.with_all_cameras_async(|camera| async move {
        camera.preset_recall_async(PresetNumber::new(1).unwrap()).await
    }).await;
    ...
    for stat in stats {
        println!("  {}: {} successful, {} failed operations",
            stat.info.id,
            stat.successful_ops,
            stat.failed_ops
        );
