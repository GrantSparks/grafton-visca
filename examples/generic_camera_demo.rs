...
fn _print_camera_info<P: Profile>(camera: &Camera<P, impl UnifiedTransport>) {
...
fn _configure_nd_filter<P, T>(_camera: &Camera<P, T>)
where
    P: Profile + NDFilter,
    T: UnifiedTransport,
{
    println!(
        "Camera {} has ND filter mode: {:?}",
        P::MODEL_NAME,
        P::ND_MODE
    );
...
fn main() -> Result<(), Error> {
    ...
    // let camera = _G2Camera::new(transport);
    ...
    // _configure_nd_filter(&camera); // Compile error!
