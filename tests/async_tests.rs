...
mod async_tests {
    ...
    async fn test_concurrent_commands() {
        ...
        let task1 = tokio::spawn(async move { t1.send_command(&PanTilt::Home).await });

        let t2 = transport.clone();
        let task2 = tokio::spawn(async move { t2.send_command(&Zoom::Stop).await });
    ...
    async fn test_error_propagation() {
        ...
        transport
            ...
            .await
            .unwrap();
        transport.send_command(&Zoom::Stop).await.unwrap();

        // Third command should fail
        let result = transport.send_command(&PanTilt::Home).await;
    ...
    async fn test_async_command_sequence() {
        ...
        let home_response = transport.send_command(&PanTilt::Home).await.unwrap();
        ...
        let zoom_response = transport.send_command(&Zoom::Stop).await.unwrap();
