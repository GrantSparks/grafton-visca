...
use grafton_visca::{AsyncViscaClient, ViscaResponse, ViscaError, AsyncViscaTransport, ViscaCommand};
...
impl MockTransport {
    ...
    async fn get_sent_commands(&self) -> Vec<Vec<u8>> {
        self.sent_commands.lock().await.clone()
    }
}
...
async fn test_semaphore_limiting() {
    ...
    let mut results: Vec<_> = futures::future::join_all(handles).await
        .into_iter()
        .map(|r| r.unwrap())
        .collect();
