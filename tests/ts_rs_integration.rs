...
fn test_transport_options_export() {
    let ts = TransportOptions::export_to_string().expect("Failed to export TransportOptions");
    assert!(!ts.is_empty());
...
fn test_profile_id_export() {
    let ts = ProfileId::export_to_string().expect("Failed to export ProfileId");
    assert!(!ts.is_empty());
}

#[test]
fn test_pan_tilt_direction_export() {
    let ts = PanTiltDirection::export_to_string().expect("Failed to export PanTiltDirection");
    assert!(!ts.is_empty());
}

#[test]
fn test_speed_level_export() {
    let ts = SpeedLevel::export_to_string().expect("Failed to export SpeedLevel");
    assert!(!ts.is_empty());
}

#[test]
fn test_zoom_position_export() {
    let ts = ZoomPosition::export_to_string().expect("Failed to export ZoomPosition");
    assert!(!ts.is_empty());
}

#[test]
fn test_preset_number_export() {
    let ts = PresetNumber::export_to_string().expect("Failed to export PresetNumber");
    assert!(!ts.is_empty());
}
...
fn test_profile_group_export() {
    ...
    let ts = ProfileGroup::export_to_string().expect("Failed to export ProfileGroup");
    assert!(!ts.is_empty());
}
...
fn test_value_types_export() {
    ...
    assert!(!GainLevel::export_to_string()
        .expect("Failed to export GainLevel")
        .is_empty());
    assert!(!GainLimit::export_to_string()
        .expect("Failed to export GainLimit")
        .is_empty());
    assert!(!IrisLevel::export_to_string()
        .expect("Failed to export IrisLevel")
        .is_empty());
    assert!(!BrightnessLevel::export_to_string()
        .expect("Failed to export BrightnessLevel")
        .is_empty());
    assert!(!ContrastLevel::export_to_string()
        .expect("Failed to export ContrastLevel")
        .is_empty());
    assert!(!ColorTemp::export_to_string()
        .expect("Failed to export ColorTemp")
        .is_empty());
    assert!(!FocusPosition::export_to_string()
        .expect("Failed to export FocusPosition")
        .is_empty());
    assert!(!PanPosition::export_to_string()
        .expect("Failed to export PanPosition")
        .is_empty());
    assert!(!TiltPosition::export_to_string()
        .expect("Failed to export TiltPosition")
        .is_empty());
    assert!(!PanSpeed::export_to_string()
        .expect("Failed to export PanSpeed")
        .is_empty());
    assert!(!TiltSpeed::export_to_string()
        .expect("Failed to export TiltSpeed")
        .is_empty());
    assert!(!MotionSyncSpeed::export_to_string()
        .expect("Failed to export MotionSyncSpeed")
        .is_empty());
    assert!(!DefogLevel::export_to_string()
...
fn test_enum_types_export() {
    ...
    assert!(!FStop::export_to_string()
        .expect("Failed to export FStop")
        .is_empty());
    assert!(!NdiQuality::export_to_string()
        .expect("Failed to export NdiQuality")
        .is_empty());
    assert!(!NoiseReductionStrength::export_to_string()
