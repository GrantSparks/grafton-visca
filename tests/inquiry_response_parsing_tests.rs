...
fn test_parse_power_inquiry_responses() {
    ...
    let result = parse_response(&data, &ResponseType::Power);
    assert!(
        result.is_ok(),
...
fn test_parse_pan_tilt_position_inquiry() {
    ...
    let result = parse_response(&data, &ResponseType::PanTiltPosition);
    assert!(
        result.is_ok(),
...
fn test_parse_zoom_position_inquiry() {
    ...
    let result = parse_response(&data, &ResponseType::ZoomPosition);
    assert!(
        result.is_ok(),
...
fn test_parse_focus_position_inquiry() {
    ...
    let result = parse_response(&data, &ResponseType::FocusPosition);
    assert!(
        result.is_ok(),
...
fn test_parse_backlight_inquiry() {
    let data = vec![0x90, 0x50, 0x02, 0xFF];
    let result = parse_response(&data, &ResponseType::Backlight);
    assert!(
        result.is_ok(),
...
fn test_parse_image_flip_inquiry() {
    // Both flips on
    let data = vec![0x90, 0x50, 0x03, 0xFF];
    let result = parse_response(&data, &ResponseType::ImageFlip);
    assert!(
        result.is_ok(),
...
fn test_parse_brightness_inquiry() {
    ...
    let result = parse_response(&data, &ResponseType::Bright);
    assert!(
        result.is_ok(),
...
fn test_parse_gain_level_inquiry() {
    ...
    let result = parse_response(&data, &ResponseType::Gain);
    assert!(
        result.is_ok(),
...
fn test_parse_contrast_inquiry() {
    ...
    let result = parse_response(&data, &ResponseType::Contrast);
    assert!(
        result.is_ok(),
...
fn test_parse_luminance_inquiry() {
    ...
    let result = parse_response(&data, &ResponseType::Luminance);
    assert!(
        result.is_ok(),
...
fn test_parse_resolution_inquiry() {
    ...
    let result = parse_response(&data, &ResponseType::Resolution);
    assert!(
        result.is_ok(),
...
fn test_parse_sharpness_inquiry() {
    let data = vec![0x90, 0x50, 0x00, 0x00, 0x00, 0x08, 0xFF];
    let result = parse_response(&data, &ResponseType::Sharpness);
    assert!(
        result.is_ok(),
...
fn test_parse_iris_inquiry() {
    ...
    let result = parse_response(&data, &ResponseType::Iris);
    assert!(
        result.is_ok(),
...
fn test_parse_shutter_inquiry() {
    ...
    let result = parse_response(&data, &ResponseType::Shutter);
    assert!(
        result.is_ok(),
...
fn test_parse_noise_reduction_2d_inquiry() {
    let data = vec![0x90, 0x50, 0x03, 0xFF];
    let result = parse_response(&data, &ResponseType::NoiseReduction2D);
    assert!(
        result.is_ok(),
...
fn test_parse_noise_reduction_3d_inquiry() {
    ...
    let result = parse_response(&data, &ResponseType::NoiseReduction3D);
    assert!(
        result.is_ok(),
...
fn test_parse_saturation_inquiry() {
    let data = vec![0x90, 0x50, 0x00, 0x00, 0x00, 0x0A, 0xFF];
    let result = parse_response(&data, &ResponseType::Saturation);
    assert!(
        result.is_ok(),
...
fn test_parse_hue_inquiry() {
    let data = vec![0x90, 0x50, 0x00, 0x00, 0x00, 0x07, 0xFF];
    let result = parse_response(&data, &ResponseType::Hue);
