...
impl TransportExpectation {
    ...
    pub fn will_ack(&mut self, socket: u8) -> &mut Self {
        self.responses.push(MockResponse::Immediate(vec![0x90, 0x40 | socket, 0xFF]));
        self
    }
    
    /// Expect a completion response after ACK
    pub fn then_complete(&mut self, socket: u8) -> &mut Self {
        self.responses.push(MockResponse::Immediate(vec![0x90, 0x50 | socket, 0xFF]));
        self
    }
...
impl BlockingTransport for MockTransport {
    fn send(&mut self, data: &[u8]) -> Result<()> {
        ...
        if inner.validate_frames && data.len() >= 3 {
            if data[0] & 0xF0 != 0x80 {
                return Err(Error::InvalidState(format!("Invalid address byte: {:02X}", data[0])));
            }
