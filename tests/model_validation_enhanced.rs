...
use grafton_visca::{
    ...
    command::{
        ...
        preset::{PresetCommand, PresetAction},
        Command,
    },
    profiles::{PTZOpticsG2, CameraModel},
    types::{ZoomPosition, FocusPosition, PresetId},
    timeout::CommandCategory,
    Error,
};
use crate::common::{
    MockTransport, MockTransportBuilder, ProtocolValidator, ValidationMode, patterns, ScenarioBuilder, MockResponse, CommandFixtures
};
...
fn assert_invalid_for_g2<C: Command>(command: &C) {
    assert!(command.validate_for_model(CameraModel::PTZOpticsG2).is_err());
}
...
fn test_power_commands_g2_validation() {
    ...
    let power_standby = PowerCommand { power: Power::Standby };
...
fn test_zoom_commands_g2_validation_comprehensive() {
    ...
    camera.zoom_out_variable(ZoomSpeed::new(3).unwrap()).unwrap();
...
fn test_pan_tilt_commands_g2_validation() {
    ...
    for pan_speed in 0x01..=0x18 {  // G2 max pan speed
        for tilt_speed in 0x01..=0x14 {  // G2 max tilt speed
...
fn test_preset_commands_g2_validation() {
    ...
    for preset_num in valid_presets {
        ...
        let set_cmd = PresetCommand { action: PresetAction::Set, preset: preset_id };
        let recall_cmd = PresetCommand { action: PresetAction::Recall, preset: preset_id };
...
fn test_command_categories_for_g2() {
    ...
    assert_eq!(PowerCommand { power: Power::On }.command_category(), CommandCategory::Quick);
    assert_eq!(ZoomCommand::Stop.command_category(), CommandCategory::Movement);
    assert_eq!(FocusCommand::Auto.command_category(), CommandCategory::Movement);
    
    let preset_cmd = PresetCommand {
        action: PresetAction::Recall,
        preset: PresetId::new(1).unwrap(),
    };
...
fn test_comprehensive_g2_command_sequence() {
    ...
    let scenario = ScenarioBuilder::new("Comprehensive G2 Test")
        ...
        .expect_home()
        .expect_command(
            &ZoomCommand::Position(ZoomPosition::new(0x4000).unwrap()).to_bytes().unwrap(),
            vec![
                MockResponse::Immediate(patterns::responses::ACK_1.to_vec()),
                MockResponse::Immediate(patterns::responses::COMPLETE_1.to_vec()),
            ]
