...
use grafton_visca::{ViscaSession, ViscaResponse, ViscaError, ViscaInquiryResponse};
use std::sync::Arc;
...
async fn test_session_inquiry_response() {
    ...
    let socket_id = {
        let mut session = session.lock().await;
        session.assign_socket(Some(grafton_visca::ViscaResponseType::ZoomPosition)).unwrap()
    };
    ...
    let inquiry_response = vec![0x90, 0x50 | socket_id, 0x01, 0x02, 0x03, 0x04, 0xFF];
    {
        ...
        match result {
            Some((sid, ViscaResponse::InquiryResponse(ViscaInquiryResponse::ZoomPosition { position }))) => {
