...
// Standard `Connect::open_tcp::<PtzOpticsG2, _>(..)` construction names the
// profile once and binds it at compile time, so neither the camera the session
// owns nor the camera it gives up can change profile.
//
