...
impl Transport for FailingMockTransport {
    ...
    fn receive_response(
        &mut self,
    ) -> grafton_visca::transport::TransportFuture<'_, (grafton_visca::types::SocketId, Vec<u8>)>
    {
        Box::pin(async move {
            // Check if we should fail
            if self.should_fail.load(Ordering::SeqCst) {
...
async fn test_basic_reconnection() {
    ...
    assert_ok(
        reconnecting
            .send_command(&cmd, grafton_visca::types::SocketId::SOCKET_0)
            .await,
...
async fn test_concurrent_operations_during_reconnection() {
    ...
    let handle1 = tokio::spawn(async move {
        reconnecting
            .send_command(&cmd1, grafton_visca::types::SocketId::SOCKET_0)
            .await
    });

    let handle2 = tokio::spawn(async move {
        reconnecting2
            .send_command(&cmd2, grafton_visca::types::SocketId::SOCKET_1)
            .await
    });
