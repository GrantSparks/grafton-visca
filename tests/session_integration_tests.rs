...
fn test_rapid_command_timing() {
    ...
    for i in 0..5 {
        ...
        device.execute_command(&PowerCommand { power: if i % 2 == 0 { Power::On } else { Power::Standby } }).unwrap();
    }
    
    let total_time = _start.elapsed();
    assert!(total_time < Duration::from_secs(2), "Commands took too long: {:?}", total_time);
