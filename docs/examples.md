...
| Operation handles | `operation_handles`, `operation_handles_async` | Demonstrate blocking and Tokio submission, exact applied waits, physical settled waits, explicit detach paired with a bounded stop, and a session that is closed on every path so a dropped movement handle stops its axes. |
