...
| ACK socket assignment         | A named ACK is exact when its socket is free. If a raw camera names a locally occupied socket or an inert exact-socket quarantine, that authoritative reuse supersedes the stale local evidence and retains its deadline only as an unkeyed quarantine (#721/#750); inventing the other socket would misroute cancellation and completion. Sequence-correlated Sony retains the 1.x other-free-socket fallback (#620/#682). Socketless ACKs retain first-free compatibility. | `src/runtime/engine/tests.rs`: `raw_socket_assignment_never_falls_back_from_an_occupied_named_socket`, `raw_ack_reusing_an_occupied_socket_displaces_the_stale_owner`, `raw_ack_reusing_a_quarantined_socket_downgrades_the_exact_hold`; `tests/issue_721_raw_socket_reuse.rs`; `tests/issue_565_transport_faults_blocking.rs` and `tests/issue_565_transport_faults_async.rs` (sequenced fallback) |
