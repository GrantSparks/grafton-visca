...
| --- | --- | --- |
| Raw positional-candidate gate and evidence-only ACK/error routing (`Sending`/`AwaitingAck`/`AwaitingCompletion`/`AwaitingLateAck`; normally one candidate, with one audited two-candidate `Urgent` exception whose ACK/error binds to neither; `AwaitingCompletion` is the completion-only, socketless shape that holds the target channel exclusively; no command FIFO or temporal recency; inquiry FIFO only when eligible; a raw named ACK supersedes a stale local owner of that socket and quarantines it, while socketless ACKs retain first-free compatibility) | `src/runtime/engine/mod.rs`: `raw_command_gate_blocks`, `raw_urgent_gate_bypass_available`, `unique_raw_command_candidate`, `resolve_raw`, `assign_ack_socket` | `src/runtime/engine/tests.rs`: `raw_gate_serializes_pre_ack_while_sony_allows_pipeline`, `urgent_raw_command_bypasses_preack_gate_and_ambiguous_ack_binds_neither`, `lost_raw_ack_makes_ordinary_first_dispatch_a_timed_wait`, `raw_error_policy_requires_unique_socketless_evidence`, `raw_ack_in_awaiting_ack_uses_the_unique_command_candidate`, `raw_socket_assignment_never_falls_back_from_an_occupied_named_socket`, `raw_ack_reusing_an_occupied_socket_displaces_the_stale_owner`, `late_ack_ambiguity_keeps_capacity_and_correlation_until_quarantine`; `tests/issue_714_async_raw_emergency_stop.rs`; `tests/issue_673_blocking_raw_emergency_stop.rs` |
