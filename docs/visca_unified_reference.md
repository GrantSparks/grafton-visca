...
* **PTZOptics 20× SDI (Gen2):** Uses a similar scale for optical zoom: `0x0000` = wide, `0x4000` = 20× optical tele. Current crate profile sources and hardware notes do not establish support for the VISCA digital zoom toggle opcode on PTZOptics built-ins, so the high-level typed API keeps digital zoom toggle and optical-plus-digital positioning unavailable for those profiles. Raw command experiments remain possible outside the profile-safe API contract.
...
* **White Balance Color Temperature:** If the camera supports a “WB Color Temp” mode (on Sony this is WB mode 0x20), the color temperature is set via `04 20 0p 0q FF` (p,q form a 2-byte value). Typically range is 2500K to 8000K mapped linearly to 0x0000–0x0037 (0 = 2500K, 0x0037 ≈ 8000K). Sony references such as BRC-H900 and EVI-H100 list this family of commands, while the FR7 uses different white-balance concepts. The high-level typed API exposes color-temperature methods only for built-in profiles that implement `HasColorTemperature`; raw command construction remains available for experiments outside the profile-safe contract.
...
* **Iris:** The current PTZOptics built-in profiles do not expose typed iris control, iris-priority mode, or iris inquiry. Their runtime metadata reports no iris range, and the profile-safe typed API requires `HasIrisControl` before those methods exist.
