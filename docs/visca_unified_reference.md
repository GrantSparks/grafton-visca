...
**Implementation Tip:** Provide a configuration switch or auto-detection in your software to choose between “Sony‑style encapsulation” and “Raw VISCA” modes. This ensures you only add the header for devices that require it. For example, an Avonic or Marshall camera can often be put into Sony-compatible mode (encapsulated, port 52381) or a raw mode (port 1259). A PTZOptics camera should always use raw mode. Some control systems (e.g., OBS plugins, Bitfocus Companion) explicitly let you select “Sony VISCA” vs “PTZOptics” to handle this difference.
