...
| Command | Bytes (broadcast) | Purpose | When to send |
|---------|------------------|---------|--------------|
| Address Set | `88 30 01 FF` | Daisy‑chain address assignment. Cameras take IDs 1…N in cable order and the last camera returns `88 30 0N FF` to tell the controller how many were found. | Once at start‑up, or after a "Network Change" message. |
| I/F Clear | `88 01 00 01 FF` | Clears command buffers in every device, cancels any motions, resets sockets. | Immediately after Address Set, or to recover from errors. |

> **Tip (serial only):** Always send `Address Set` then `I/F Clear` after power‑up or hot‑plug so every camera knows its ID and the command state is clean. These commands are **not** used in VISCA‑over‑IP (IP devices always use ID 1).
...
| Phase | Command (`⟶`) | Inquiry (`?`) |
|-------|---------------|---------------|
| Controller ⟶ Camera | `8x 01 … FF` | `8x 09 … FF` |
| Camera immediate | **ACK** `9x 41 FF` (locks socket) | *(none)* |
| Camera finished | **Completion** `9x 51 FF` | **Data reply** `9x 50 <data…> FF` |
| Errors | `9x 6Y EE FF` | Same |

### 4.2 Return Messages & Error Codes

| Reply | Hex Pattern | Meaning |
|-------|-------------|---------|
| ACK | `9x 4y FF` | Accepted into socket *y* (1 or 2). |
| Completion | `9x 5y FF` | Command finished; socket *y* freed. |
| Data reply | `9x 50 … FF` | Inquiry result (no prior ACK). |
| Syntax Error | `9x 60 02 FF` | Bad opcode/parameter. |
| Buffer Full | `9x 60 03 FF` | Both sockets busy. |
| Command Canceled | `9x 6y 04 FF` | Prior cmd in socket *y* aborted. |
| No Socket | `9x 6y 05 FF` | "Cancel" sent for empty socket. |
| Not Executable | `9x 6y 41 FF` | Camera currently cannot execute cmd. |
...
## 5  Inquiry Command Set (Universally Recognised)

All inquiries replace the byte `01` (command) with `09`. Replies use `50` and contain the requested data.  

| Category | Inquiry | Bytes Sent | Typical Reply | Data Meaning | Sony | PTZOptics | FR7 |
|----------|---------|-----------|---------------|--------------|------|-----------|-----|
| Power State | PowerInq | `8x 09 04 00 FF` | `9x 50 02/03 FF` | 02 = On, 03 = Standby | ✔ | ✔ | ✔ |
| Zoom Position | ZoomPosInq | `8x 09 04 47 FF` | `9x 50 0p 0q 0r 0s FF` | 16‑bit zoom | ✔ | ✔ | ✔ |
| Focus Mode | FocusModeInq | `8x 09 04 38 FF` | `9x 50 02/03 FF` | 02 = Auto, 03 = Manual | ✔ | ✔ | ✔ |
| Focus Position | FocusPosInq | `8x 09 04 48 FF` | `9x 50 0p 0q 0r 0s FF` | 16‑bit focus | ✔ | ✔ | ✔ |
| Pan/Tilt Position | PTPosInq | `8x 09 06 12 FF` | `9x 50 PP PP PP PP TT TT TT TT FF` | 16‑bit pan & tilt | ✔ | ✔ | ✔ |
| Exposure Mode | AEModeInq | `8x 09 04 39 FF` | `9x 50 00/03/0A/0B/0D FF` | 00 = Full Auto … | ✔ | ✔ | ✔ |
| Shutter Speed | ShutterInq | `8x 09 04 4A FF` | `9x 50 0p 0q 0r 0s FF` | 16‑bit shutter idx | ✔ | ✔ | ✔ |
| Iris Level | IrisInq | `8x 09 04 4B FF` | `9x 50 0p 0q 0r 0s FF` | 16‑bit iris | ✔ | ✔ | ✔ |
| Gain | GainInq | `8x 09 04 4C FF` | `9x 50 0p 0q 0r 0s FF` | 16‑bit gain | ✔ | ✔ | ✔ |
| WB Mode | WBModeInq | `8x 09 04 35 FF` | `9x 50 00/01/02/04/05/20 FF` | Auto, Indoor, … | ✔ | ✔ | ✔ |
| Colour Temp | ColorTempInq | `8x 09 04 20 FF` | `9x 50 0p 0q FF` | mired value | ✔ | ✔ | ✖ |
| Tally State | TallyInq | `8x 09 7E 01 0A 00 FF` | `9x 50 02/03 FF` | 02 = Red On | ✔ | ✖ | ✔* |
| Device Info | VersionInq | `8x 09 00 02 FF` | `9x 50 VV VV MM MM FF FF KK FF` | Vendor, Model, FW, Sockets | ✔ | ✔ | ✔ |

\* FR7 has separate Red/Green tally inquiries (`7E 01 0A 00` and `7E 04 1A 00`).  
Almost every VISCA camera supports these baseline inquiries; if a feature doesn't exist the camera usually returns `… 6y 41 FF`.

---

## 6  Baseline Command Set (universally recognised)
...
## 10  Behavioural Edge‑Cases & Recommended Tests
...
8. Send Address Set + I/F Clear after any Network Change – ensure proper camera addressing and clean command state after power-up, hot-plug, or any network topology changes.
...
* **Sony – "VISCA Protocol Spec v1.4"** (2004) pages 26‑31.  
  — Inquiry/IF_Clear tables and comprehensive error code documentation.

---

### Appendix A – CSV Opcode Table

A machine‑parsable CSV listing of opcodes (column order: **Category, Mnemonic, Opcode, Bytes, Sony, PTZOptics, FR7, Notes**) is provided below. This includes both Command and Inquiry opcodes.
...
PanTilt,PTPosInq,81 09 06 12 FF,5,Yes,Yes,Yes,Reply: 90 50 PP PP PP PP TT TT TT TT FF  
