    ...
    T: UnifiedTransport,
{
    ...
    /// Check if a feature is supported by this camera profile.
    #[must_use]
    pub fn has_feature(&self, feature: CameraFeature) -> bool {
        // For now, we can't determine feature support at compile time
        // without a ProfileIntrospection trait that profiles can implement
...
/// Extension trait for feature detection.
    ...
    T: UnifiedTransport,
{
    fn supports_feature(&self, feature: CameraFeature) -> bool {
        // For compile-time profiles, we check against known mandatory features
        match feature {
...
where
    ...
    T: UnifiedTransport,
{
    /// Send a VISCA command to the camera without expecting a response.
    pub fn send_command_no_response<C: EncodeVisca>(&self, command: &C) -> Result<(), Error> {
        let mut buffer = [0u8; 64];
        let size = command.encode_into(self.camera_id, &mut buffer)?;
        let cmd_bytes = &buffer[..size];

        // Apply protocol-specific framing
        let framed_bytes = self.envelope.frame_command(cmd_bytes, false);
        self.transport.send_blocking(&framed_bytes)?;
        Ok(())
    }

    /// Send a VISCA command to the camera and wait for a response.
    pub fn send_command_sync<C: EncodeVisca>(
        &self,
        command: &C,
        _expected_response: crate::command::ResponseType,
    ) -> Result<Response, Error> {
        // Use the blocking method
        self.send_command_blocking(command)
    }

    /// Send raw bytes to the camera.
    ///
    /// # Safety
    /// This bypasses all validation and can send invalid commands.
    /// Use with caution and only when implementing custom commands.
    pub fn send_raw(&self, data: &[u8]) -> Result<(), Error> {
...
where
    ...
    T: UnifiedTransport,
{
    /// Send a command to the camera using blocking method.
    /// This is used internally by the blocking trait implementations.
    pub(crate) fn send_command_blocking<C>(&self, command: &C) -> Result<Response, Error>
    where
        C: EncodeVisca,
    {
        // Always use the blocking implementation
        self.send_command_blocking_impl(command)
    }
}

// Impl block for convenience constructor with Transport types
#[cfg(not(feature = "async"))]
impl<P, AT> Camera<P, crate::transport::AsyncTransportWrapper<AT>>
where
    P: Profile,
    AT: crate::transport::core::Transport + Send + Sync + 'static,
    for<'a> AT::SendFut<'a>: Send,
    for<'a> AT::RecvFut<'a>: Send,
{
    /// Create a new camera with a transport implementing the Transport trait.
    ///
    /// This is a convenience method for blocking mode.
    pub fn new(transport: AT) -> Self {
        let wrapped = crate::transport::AsyncTransportWrapper { transport };
        Self::from_transport(wrapped)
    }
}

// Re-export Camera type at module level for backward compatibility
pub use Camera as GenericCamera;
