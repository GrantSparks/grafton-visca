//! VISCA transport session manager that works with both blocking and async transports.
//!
...
///
/// This is the main transport type that users interact with. It wraps a transport
    ...
    impl<T: Transport> ViscaTransport<T> {
        ...
        pub fn send_command(&mut self, command: &dyn Command) -> Result<Response, Error> {
            ...
            let final_response = loop {
                let response_data = match self.inner.receive() {
                    Ok(data) => data,
                    Err(e) => {
                        self.release_socket(socket_id);
                        return Err(e);
...
mod async_impl {
    ...
    impl<T: AsyncTransport + Debug> ViscaTransport<T> {
