...
use crate::{
    command::{
        image::SharpnessMode, resolution::NdFilterPosition, resolution::PictureEffectMode,
        AutoWhiteBalanceSensitivity, FocusRange, InquiryData,
    },
    error::Error,
    types::DefogLevel,
};
...
// =============================================================================
// Parse functions used by ViscaInquiry derive macro
// =============================================================================
// These functions are called by code generated by the ViscaInquiry macro in
// grafton-visca-macros. They provide parse_response() implementations for
// inquiry structs that use the `parser = "custom"` attribute.
//
// Each function delegates to the underlying type-safe parsing infrastructure
// where possible, using BoolConvention for boolean fields.
// =============================================================================
...
pub fn parse_sharpness_mode(data: &[u8]) -> Result<InquiryData, Error> {
    ...
    match data[0] {
        ...
        _ => Err(Error::InvalidParameter {
            ...
            value: Cow::Owned(format!("0x{:02X}", data[0])),
            reason: Cow::Borrowed("Expected 0x02 (Auto) or 0x03 (Manual)"),
        }),
    }
}

/// Parse menu open/close status (inverted: 0x02 = open, 0x03 = closed)
...
pub fn parse_tally_status(data: &[u8]) -> Result<InquiryData, Error> {
    ...
    let red_payload = Payload::new(&data[0..1]);
    let green_payload = Payload::new(&data[1..2]);
...
/// Parse night/day mode status (standard: 0x02 = day, 0x03 = night)
...
/// Parse standby mode status (standard: 0x02 = active, 0x03 = standby)
...
/// Parse green tally light status (inverted: 0x02 = on, 0x03 = off)
...
pub fn parse_nd_filter(data: &[u8]) -> Result<InquiryData, Error> {
    ...
    Ok(InquiryData::NdFilter {
        position: NdFilterPosition::from_byte(data[0]),
    })
}
...
pub fn parse_picture_effect(data: &[u8]) -> Result<InquiryData, Error> {
    ...
    Ok(InquiryData::PictureEffect {
        effect: PictureEffectMode::from_byte(data[0]),
    })
}

/// Parse iris control mode (standard: 0x02 = manual, 0x03 = auto)
...
/// Parse defog mode (standard: 0x02 = off, 0x03 = on)
...
/// Parse digital PTZ mode (standard: 0x02 = off, 0x03 = on)
...
pub fn parse_defog_level(data: &[u8]) -> Result<InquiryData, Error> {
    ...
    let level = DefogLevel::new(data[0]).map_err(|_| Error::InvalidParameter {
    ...
    let sensitivity = match data[0] {
        ...
        _ => {
            return Err(Error::InvalidParameter {
                ...
                value: Cow::Owned(format!("0x{:02X}", data[0])),
                reason: Cow::Borrowed("Expected 0x00 (High), 0x01 (Normal), or 0x02 (Low)"),
            })
        }
    };
...
/// Parse auto trace mode (standard: 0x02 = off, 0x03 = on)
...
/// Parse focus unlock state (standard: 0x02 = locked, 0x03 = unlocked)
...
/// Parse auto focus status (standard: 0x02 = off, 0x03 = on)
